	...
	<div class="page">
		<div class="container">
			...
			<div class="wrap" @keyup.enter="signIn">
			    <input type="text" placeholder="name" autofocus v-model="name">
			    <input type="pwd" placeholder="pwd" v-model="pwd">
...
<script>
	...
	export default {
		...
		methods: {
			signIn : function() {
				var _this = this;
				var param = {name : this.name, pwd : this.pwd};
				var resource = this.$resource('http://localhost:3000/judgeLoading');
				resource.save(param).then((response) => {
					if(response.body.code == 200) {
					    console.log('200到了');
						sessionStorage.setItem('name', _this.name);
						this.$router.push('/gallery/' + _this.name);
					}
					else {
